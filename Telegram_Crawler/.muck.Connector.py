...
class TelegramOperator:
    ...
    async def initialize_run(self):
        ...
        print("     ->[*] Initializing the dialogs...")
        #dialogs = asyncio.run(self.client.get_dialogs(limit=5000))
        #dialogs = self.client.loop.run_until_complete(get_dialogs(limit=5000))

        dialogs = await self.client.get_dialogs(limit=5000)
        # print(dialogs)
        # print(dialogs.__name__)
        self.groups = list()
        for dialog in dialogs:
            try:
...
async def main():
    ...
    while(count<tgtRep):
        top = await TelegramOperator() #Object is recreated every time to reset the data
